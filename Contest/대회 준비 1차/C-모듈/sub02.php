<?php require_once "db.php" ?>

<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Document</title>
  <link
    rel="stylesheet"
    href="./선수제공파일/공통/fontawesome/css/font-awesome.min.css" />
  <link rel="stylesheet" href="http://127.0.0.1/css/sub02.css" />
</head>

<body>
  <div class="loading-container">
    <div class="loading"></div>
    <div class="background"></div>
  </div>

  <div class="log">
    방금 비회원 <span class="userName">님</span>님이 <span class="userPrice">원</span>을 결제하셨습니다
  </div>

  <header>
    <nav>
      <a href="./index.php"><img src="../imgs/logo.png" alt="logo" /></a>
      <ul class="menu">
        <li><a href="./sub01.php">소개</a></li>
        <li>
          <a href="#">판매 상품</a>
          <ul>
            <li><a href="./sub02.php">전체상품</a></li>
            <li><a href="#">인기상품</a></li>
          </ul>
        </li>
        <li><a href="#">가맹점</a></li>
        <li><a href="#">장바구니</a></li>
      </ul>
      <ul class="menu">
        <li><a href="./sub04.php">장바구니</a></li>

        <?php if (($_SESSION["ss"]->id ?? "") === "admin") { ?>
          <li><a href="#">admin</a></li>
          <li><a href="./logout.php">로그아웃</a></li>
        <?php } else if ($_SESSION["ss"]->id ?? false) { ?>
          <li><a href="#"><?= htmlspecialchars($_SESSION["ss"]->id) ?>님</a></li>
          <li><a href="./logout.php">로그아웃</a></li>
        <?php } else { ?>
          <li><a class="login">로그인</a></li>
          <li><a class="signup">회원가입</a></li>
        <?php } ?>
      </ul>
    </nav>
  </header>

  <main>
    <div class="video-Box">
      <div class="video">
        <video src="./선수제공파일/B-Module/AD.mp4" autoplay></video>
      </div>
      <div class="video-ctrl">
        <div class="control">
          <div class="start">재생</div>
          <div class="stop">멈춤</div>
          <div class="pause">일시정지</div>
          <div class="back">10초 뒤로</div>
          <div class="forward">10초 앞으로</div>
          <div class="10plus">배속(0.1배)</div>
          <div class="10minus">감속(0.1배)</div>
          <div class="speed-Back">배속 초기화</div>
          <div>
            <input type="checkbox" id="loop" />
            <label class="loop" for="loop">반복 켜기 / 끄기</label>
          </div>
          <div>
            <input type="checkbox" id="autoplay" checked />
            <label class="autoplay" for="autoplay">자동재생 켜기 / 끄기</label>
          </div>
        </div>

        <div class="hide">
          <input type="checkbox" id="hide" />
          <label class="ctrl-hide" for="hide">컨트롤러 보이기 / 숨기기</label>
        </div>
      </div>
    </div>

    <div class="allProduct">
      <div class="container">
        <div class="itemBox">
          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/건강식품/1.PNG"
                alt="건강식품-Img1" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 이뮨 멀티비타민&amp;미네랄</p>
                </div>
              </div>

              <div>
                <p class="fa price">
                  &#xf155; 가격
                  <span style="text-decoration: line-through">75,000</span>-&gt; <span class="price">
                    65,000
                  </span>
                </p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/건강식품/2.PNG"
                alt="건강식품-Img2" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 센트룸</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 27,000</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/건강식품/3.PNG"
                alt="건강식품-Img3" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 닥터브라이언</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 2,000</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/건강식품/4.PNG"
                alt="건강식품-Img4" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 액티브 멀티포맨</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 30,000</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/건강식품/5.PNG"
                alt="건강식품-Img5" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 네이처메이드B12</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 30,000</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="container">
        <div class="itemBox">
          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/디지털/4.PNG"
                alt="디지털-Img4" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 파이널마우스 스타라이트12 페가수스 미디엄</p>
                </div>
              </div>

              <div>
                <p class="fa price">
                  &#xf155; 가격
                  <span style="text-decoration: line-through">1,254,000</span>-&gt; <span class="price">1,128,600 </span>
                </p>
              </div>
              <div>
                <p>일반배송 3,000원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/디지털/1.PNG"
                alt="디지털-Img1" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: PANTONE PD충전 보조배터리</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 24,400</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/디지털/2.PNG"
                alt="디지털-Img2" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: Bowie D05 무선 블루투스 5.3 헤드셋</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 36,900</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/디지털/3.PNG"
                alt="디지털-Img3" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 독거미 F99 기계식 키보드</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 70,750</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/디지털/5.PNG"
                alt="디지털-Img5" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 보이저5200 블루투스 이어폰</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 146,000</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="container">
        <div class="itemBox">
          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/팬시/4.PNG"
                alt="팬시-Img4" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 게이밍 이어폰 VJJB NI</p>
                </div>
              </div>

              <div>
                <p class="fa price">
                  &#xf155; 가격
                  <span style="text-decoration: line-through">38,900</span>-&gt;2<span class="price">28,900 </span>
                </p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/팬시/1.PNG"
                alt="팬시-Img1" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 명품 자동 장우산</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 31,600</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/팬시/2.PNG"
                alt="팬시-Img2" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 14K 윙블링 원터치 링 귀걸이(주문제작)</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 250,000</p>
              </div>
              <div>
                <p>일반배송 3,000원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/팬시/3.PNG"
                alt="팬시-Img3" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 14K 윙블링 메르시 목걸이(주문제작)</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 265,000</p>
              </div>
              <div>
                <p>일반배송 3,000원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/팬시/5.PNG"
                alt="팬시-Img5" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 인스탁스 미니 에보</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 320,000</p>
              </div>
              <div>
                <p>일반배송 3,000원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="container">
        <div class="itemBox">
          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/향수/4.PNG"
                alt="향수-Img4" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 몽블랑 익스플로러 EDP 60ml</p>
                </div>
              </div>

              <div>
                <p class="fa price">
                  &#xf155; 가격
                  <span style="text-decoration: line-through">103,000</span>-&gt;9<span class="price">93,000</span>
                </p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/향수/1.PNG"
                alt="향수-Img1" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 에스쁘아 솔리드 퍼퓸 4.2g</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 26,000</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/향수/2.PNG"
                alt="향수-Img2" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 호텔도슨 향수 오드퍼퓸 75ml</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 153,000</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/향수/3.PNG"
                alt="향수-Img3" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 랑방 레 플레르 EDT 90ml</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 64,500</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/향수/5.PNG"
                alt="향수-Img5" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 캘빈클라인 One EDT 50ml</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 58,900</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="container">
        <div class="itemBox">
          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/헤어케어/5.PNG"
                alt="헤어케어-Img5" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 닥터포헤어 피토프레시 헤어쿨링 스프레이 150ml</p>
                </div>
              </div>

              <div>
                <p class="fa price">
                  &#xf155; 가격
                  <span style="text-decoration: line-through">16,000</span>-&gt;1<span class="price">14,400 </span>
                </p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/헤어케어/1.PNG"
                alt="헤어케어-Img1" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 어노브 딥 데미지 트리트먼트 EX 더블</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 29,800</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/헤어케어/2.PNG"
                alt="헤어케어-Img2" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>
                    상품명: 려 루트젠 여성맞춤 볼륨 탈모증상케어 샴퓨 353ml
                  </p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 21,900</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/헤어케어/3.PNG"
                alt="헤어케어-Img3" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 라보에이치 두피쿨링&amp;노세범 샴푸 333ml</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 19,800</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>

          <div class="itemCart">
            <div class="img-box">
              <img
                src="./선수제공파일/A-Module/images/헤어케어/4.PNG"
                alt="헤어케어-Img4" />
            </div>

            <div class="text-content">
              <div>
                <div class="item-title">
                  <p>상품명: 모로칸오일 헤어트리트먼트 100ml</p>
                </div>
              </div>

              <div>
                <p class="fa price">₩ 가격: 52,200</p>
              </div>
              <div>
                <p>일반배송 2,500원 (20,000원 이상 무료배송)</p>
              </div>
              <div class="btn">
                <a href="#">구매하기</a>
                <a class="addCrat">장바구니담기 </a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="buy">비회원 주문</div>
  </main>

  <div class="modal">
    <div class="buy-container">
      <div class="item-box">
        <div class="modal-close">닫기</div>
        <div class="user-info">
          <div class="nonuser">비회원: </div>
          <div class="number">1</div>
          <div class="number">2</div>
          <div class="number">3</div>
          <div class="number">4</div>
          <div class="number">5</div>
          <div class="number">6</div>
        </div>
        <div class="title">장바구니</div>
        <div class="cataegory">
          <div>건강식품</div>
          <div>디지털</div>
          <div>팬시</div>
          <div>향수</div>
          <div>헤어케어</div>
        </div>
        <div class="items">

        </div>
      </div>

      <div class="price-container">
        <div class="drop"></div>
        <div class="total-buy">
          <div class="total">
            <span>총 가격:</span>
            <span class="totalPrice">0원</span>
          </div>

          <div class="btns">
            <div class="buy-btn">구매</div>
          </div>
        </div>
      </div>
    </div>
  </div>

  <div class="footer">
    <div class="footer-section">
      <a href="#"><img src="../imgs/logo.png" alt="footer-logo" /></a>
      <p>고객센터</p>
      <p>온라인몰 고객센터: 1580-8282</p>
      <p>매장고객센터: 1577-8254</p>
      <p>운영시간: 평일 09:00 - 18:00</p>
      <p>주말 및 공휴일은 <a href="#">1:1 문의하기</a>를 이용해주세요.</p>
    </div>

    <div class="footer-section link-and-sns">
      <div class="footer-section link">
        <a href="#">개인정보처리방침</a> | <a href="#">이용약관</a> |
        <a href="#">법적 고지</a> | <a href="#">청소년 보호방침</a> |
        <a href="#">이메일 무단수집 거부</a> |
        <a href="#">사이트맵</a>
      </div>

      <div class="footer-section link sns">
        <a href="#" class="fa fa-instagram"></a>
        <a href="#" class="fa fa-twitter"></a>
        <a href="#" class="fa fa-facebook"></a>
        <a href="#" class="fa fa-twitch"></a>
        <a href="#" class="fa fa-youtube"></a>
      </div>
    </div>

    <div class="footer-section">
      <p>(주)GIFTS:Mall</p>
      <p>사업자등록번호: 809-81-01157</p>
      <p>대표이사: 황기영</p>
      <p>주소: 서울특별시 용산구 한강대로 123, 40층</p>
      <p>본사 대표전화: 02-123-4567</p>
    </div>

    <div class="footer-section safe">
      <p>
        지방은행 구매안전서비스: GIFTS:Mall은 현금 결제한 금액에 대해
        지방은행과 채무지급보증 계약을 체결하여 안전한 거래를 보장합니다.
      </p>
      <a href="#">서비스 가입 사실 확인</a>
    </div>

    <div class="footer-section">
      <p>COPYRIGHTⓒ 2024 GIFTS:MALL KOREA INC. ALL RIGHTS RESERVED</p>
    </div>
  </div>

  <script src="../script/jquery-3.4.1.min.js"></script>
  <script src="../script/jquery-ui.min.js"></script>
  <script src="../script/sub02.js?=v<?php time() ?>"></script>


</body>

</html>